...
class DataReader():
    ...
    def read_graph_adj(self, fpath, nodes, graphs):
        ...
        for i in range(len(adj_list)):
            identity_matrix = np.eye(adj_list[i].shape[0])
            adj_list[i] = adj_list[i] + identity_matrix
        return adj_list
...
for fold_id in range(n_folds):
    ...
    if args.model == 'gcn':
        model = GCN(input_dim=loaders[0].dataset.num_features,
                           output_dim=loaders[0].dataset.num_classes,
                           hidden_dim=args.n_hidden,
                           dropout=args.dropout).to(args.device)
    elif args.model == 'g2t':
        model = Model(input_dim=loaders[0].dataset.num_features,
                                    hidden_dim=args.n_hidden,
                                    output_dim=loaders[0].dataset.num_classes,
                                    d_model=args.n_hidden,
                                    nhead=args.multi_head,
                                    num_layers=n,
                                    dropout=args.dropout).to(args.device)
    else:
